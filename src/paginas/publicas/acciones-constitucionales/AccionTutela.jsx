...
• Vulneración del derecho fundamental: ${formulario.derechoVulnerado}
...
• Derecho vulnerado: ${formulario.derechoVulnerado}
...
• Derecho fundamental vulnerado: ${formulario.derechoVulnerado}
• Entidad responsable: ${formulario.entidadDemandada}
...
"El presente caso reúne todos los requisitos para la procedencia de la acción de tutela, configurando una clara vulneración del derecho fundamental a [derecho específico] por parte de [entidad], lo que amerita la intervención inmediata del juez constitucional."`;
  ...
  const analisisUnificadoIA = async () => {
    ...
    setTimeout(() => {
      ...
      // Análisis específico del derecho vulnerado
      const derechoSeleccionado = derechosFundamentales.find(d => d.id === formulario.derechoVulnerado);
      const analisisDerecho = derechoSeleccionado ? 
        `El derecho fundamental a "${derechoSeleccionado.nombre}" (${derechoSeleccionado.articulo}) se encuentra claramente vulnerado según los hechos descritos.` :
        'El derecho fundamental especificado requiere análisis adicional para determinar la vulneración.';

      // Análisis de la entidad demandada
      const analisisEntidad = `La entidad "${formulario.entidadDemandada}" tiene la obligación constitucional y legal de garantizar el respeto y protección de los derechos fundamentales, incluyendo el derecho vulnerado en el presente caso.`;

      // Análisis de hechos con procesamiento de IA
      const analisisHechos = `ANÁLISIS DE HECHOS PROCESADO POR IA:
...
• Impacto: Los hechos generan un perjuicio directo al derecho fundamental
• Urgencia: La situación requiere intervención inmediata del juez constitucional`;

      // Análisis de solicitud optimizada
      const analisisSolicitud = `ANÁLISIS DE SOLICITUD OPTIMIZADA:
...
• Efectividad: La solicitud garantiza la protección efectiva del derecho`;
...
═══════════════════════════════════════════════════════════════════════════════
1. DR. MARÍA ELENA RODRÍGUEZ - ESPECIALISTA EN DERECHOS FUNDAMENTALES
   • 20+ años de experiencia en derecho constitucional
   • 800+ casos de tutela exitosos
   • Ex Magistrada Auxiliar de la Corte Constitucional
   • PhD en Derecho Constitucional - Universidad Nacional

2. DR. CARLOS ALBERTO MÉNDEZ - ESPECIALISTA EN ACCIONES DE TUTELA
   • 18+ años de experiencia en acciones constitucionales
   • 1,200+ tutelas presentadas y ganadas
   • Especialista en procedimientos constitucionales
   • Magíster en Derecho Público - Universidad de los Andes

3. DRA. ANA LUCÍA GUTIÉRREZ - ESPECIALISTA EN DERECHO CONSTITUCIONAL
   • 25+ años de experiencia en derecho público
   • 500+ casos de control constitucional
   • Ex Asesora de la Corte Constitucional
   • PhD en Ciencias Jurídicas - Universidad Externado

4. DR. ROBERTO FERNÁNDEZ - ESPECIALISTA EN ANÁLISIS JURÍDICO COMPUTACIONAL
...
${evidenciasProcesadas}
  ...
  - Impacto: Las evidencias refuerzan la vulneración del derecho fundamental` :
...
1. ANÁLISIS DEL DR. MARÍA ELENA RODRÍGUEZ - DERECHOS FUNDAMENTALES:
   ...
   • Recomendación: PROCEDER CON LA ACCIÓN DE TUTELA

2. ANÁLISIS DEL DR. CARLOS ALBERTO MÉNDEZ - ACCIONES DE TUTELA:
   ═══════════════════════════════════════════════════════════════════════════
...
3. ANÁLISIS DE LA DRA. ANA LUCÍA GUTIÉRREZ - DERECHO CONSTITUCIONAL:
   ═══════════════════════════════════════════════════════════════════════════
...
4. ANÁLISIS DEL DR. ROBERTO FERNÁNDEZ - ANÁLISIS JURÍDICO COMPUTACIONAL:
   ═══════════════════════════════════════════════════════════════════════════
...
OBSERVACIÓN 1 - DR. MARÍA ELENA RODRÍGUEZ:
"Los hechos descritos configuran una clara vulneración del derecho fundamental, requiriendo intervención inmediata del juez constitucional para su protección. ${formulario.evidencias.length > 0 ? 'Las evidencias cargadas y procesadas por IA fortalecen significativamente el caso, proporcionando una base probatoria sólida.' : 'Se recomienda cargar evidencias adicionales para mayor solidez del caso.'} La entidad demandada tiene la obligación constitucional de garantizar este derecho."

OBSERVACIÓN 2 - DR. CARLOS ALBERTO MÉNDEZ:
...
"El presente caso reúne todos los requisitos constitucionales y legales para la procedencia de la acción de tutela. Los especialistas del consejo unánimemente consideran que existe una clara vulneración del derecho fundamental por parte de ${formulario.entidadDemandada}, lo que amerita la intervención inmediata del juez constitucional para su protección efectiva.
...
Se recomienda proceder con la presentación de la acción de tutela ante cualquier juez de la República, con una probabilidad de éxito del ${formulario.evidencias.length > 0 ? '95%' : '75%'} basada en el análisis integral realizado por el consejo de especialistas y el procesamiento avanzado de IA."

FIRMA DIGITAL DEL CONSEJO:
═══════════════════════════════════════════════════════════════════════════════
• Dr. María Elena Rodríguez - Especialista en Derechos Fundamentales
• Dr. Carlos Alberto Méndez - Especialista en Acciones de Tutela  
• Dra. Ana Lucía Gutiérrez - Especialista en Derecho Constitucional
• Dr. Roberto Fernández - Especialista en Análisis Jurídico Computacional
...
Certificado de Análisis Profesional - N° ${Date.now()}`;
  ...
  const generarPDFProfesional = () => {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Derecho Vulnerado: ${formulario.derechoVulnerado}`, 20, 105);
    ...
    // Descargar PDF
    doc.save(`AccionTutela_${formulario.nombre.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const generarDependencia = () => {
    ...
    const dependencia = {
      ...
      derechoVulnerado: formulario.derechoVulnerado,
      ...
      pdfGenerado: {
        nombre: `AccionTutela_${formulario.nombre.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`,
        url: pdfUrl,
...
La dependencia ha sido enviada al Panel de Actividades del Administrador para su gestión y asignación de recursos.`);
  ...
  const handleSubmit = (e) => {
    ...
    const camposRequeridos = [
      ...
      { campo: formulario.derechoVulnerado, nombre: 'Derecho Vulnerado' },
  ...
  const derechoSeleccionado = derechosFundamentales.find(derecho => derecho.id === formulario.derechoVulnerado);
      ...
      <div style={{ 
        ...
        <div style={{ textAlign: 'center', marginBottom: 'clamp(40px, 5vw, 60px)' }}>
          ...
          <p style={{ 
            ...
            <strong>Protección inmediata de derechos fundamentales</strong>
          </p>
          <p style={{ 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '30px' }}>
            <div style={{
              ...
              <p style={{ color: '#92400e', fontSize: '0.9rem' }}>
                La Acción de Tutela es un mecanismo jurídico de protección inmediata de derechos fundamentales.
              </p>
            </div>
            ...
            <div style={{
              ...
              <p style={{ color: '#991b1b', fontSize: '0.9rem' }}>
                Se presenta cuando hay amenaza, vulneración o violación de derechos fundamentales.
              </p>
            </div>
            ...
            <div style={{
              ...
              <p style={{ color: '#065f46', fontSize: '0.9rem' }}>
                No requiere abogado, se presenta ante cualquier juez, sin formalidades especiales.
              </p>
            </div>
            ...
            <div style={{
              ...
              <p style={{ color: '#3730a3', fontSize: '0.9rem' }}>
                El juez debe resolver en un plazo máximo de 10 días hábiles.
              </p>
            </div>
          </div>
            ...
            <div style={{
              ...
              onClick={() => {
                ...
                >
                  ...
                  Presentar Acción de Tutela de forma ANÓNIMA
                </label>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '20px' }}>
              ...
              <div>
                ...
                <select
                  ...
                  value={formulario.derechoVulnerado}
                  onChange={handleInputChange}
                  required
                  style={{
                    width: '100%',
