...
class IaService {
  ...
  async verificarEstado() {
    try {
      const response = await iaClient.get('/consejo-ia/health');
      return {
        success: true,
        data: response.data
      };
  ...
  async verificarEstadoMedicinaNatural() {
    try {
      const response = await iaClient.get('/medicina-natural/health');
      return {
        success: true,
        data: response.data
      };
    } catch (error) {
      console.error('Error verificando estado medicina natural:', error);
      return {
        success: false,
        message: 'Servicio de medicina natural no disponible'
      };
    }
  }

  async verificarEstadoEspecialistas() {
    try {
      const response = await iaClient.get('/especialistas-ia/health');
      return {
        success: true,
        data: response.data
      };
    } catch (error) {
      console.error('Error verificando estado especialistas:', error);
      return {
        success: false,
        message: 'Servicio de especialistas no disponible'
      };
    }
  }

  async verificarEstadoAnalisisJuridico() {
    try {
      const response = await iaClient.get('/analisis-juridico/health');
      return {
        success: true,
        data: response.data
      };
    } catch (error) {
      console.error('Error verificando estado análisis jurídico:', error);
      return {
        success: false,
        message: 'Servicio de análisis jurídico no disponible'
      };
    }
  }

  async verificarEstadoAnalisisDocumentos() {
    try {
      const response = await iaClient.get('/analisis-documentos/health');
      return {
        success: true,
        data: response.data
      };
    } catch (error) {
      console.error('Error verificando estado análisis de documentos:', error);
      return {
        success: false,
        message: 'Servicio de análisis de documentos no disponible'
      };
    }
  }
}
